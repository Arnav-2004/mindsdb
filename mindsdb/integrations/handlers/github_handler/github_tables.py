    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        if query.order_by and len(query.order_by) > 0:
            ...
            for an_order in query.order_by:
                ...
                if an_order.field.parts[1] in ["author", "committer", "sha"]:
                    if commits_kwargs != {}:
                        raise ValueError(
                            "Duplicate order conditions found for author/committer/sha"
                        )
        ...
        for a_where in conditions:
            ...
            elif a_where[1] == "committer":
                if a_where[0] != "=":
                    raise ValueError("Unsupported where operation for committer")

                commits_kwargs["committer"] = a_where[2]
            elif a_where[1] == "sha":
                if a_where[0] != "=":
                    raise ValueError("Unsupported where operation for sha")

                commits_kwargs["sha"] = a_where[2]
            else:
                raise ValueError(f"Unsupported where argument {a_where[1]}")
